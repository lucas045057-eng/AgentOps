...
def ensure_airdrop_wallet_file(path: Path) -> tuple[int, int]:
    ...
    desired_total = _positive_env_int("AIRDROP_WALLET_COUNT", "SPORTIX_WALLET_COUNT", default=1000)
    created = 0
    appended = 0
    if not path.exists() or path.stat().st_size == 0:
        source_value = os.environ.get("SPORTIX_WALLET_SOURCE_FILE", "").strip()
        if source_value:
            source_path = Path(source_value).expanduser()
            if not source_path.is_file():
                raise FileNotFoundError(f"SportIX 钱包来源文件不存在：{source_path}")
            path.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source_path, path)
        elif not auto_provision:
            raise FileNotFoundError(f"AirDrop 钱包文件不存在：{path}")
        else:
            created = desired_total
            generate_wallet_file(path, created)
    elif auto_provision:
        current_rows, _, _ = load_wallet_txt(path)
        missing = max(0, desired_total - len(current_rows))
        if missing:
            generate_wallet_file(path, missing)
            created = missing
